...
const MODULE_PATH: &[&str] = &["meta_signal_message", "lib"];
const SIGNAL_MODULE_PATH: &[&str] = &["signal_message", "lib"];
const IMPORTED_SIGNAL_TYPES: &[&str] = &["MessageDaemonConfiguration"];
...
impl SchemaBuild {
    ...
    fn run(&self) {
        ...
        let ordinary_metadata = CargoEthosSourceMetadata::new("signal-message");
        ordinary_metadata.emit_dependency_rerun_instruction();
        let ordinary_source_path = ordinary_metadata
            .dependency_source_directory()
            .expect("signal-message publishes its Ethos source directory")
            .join("interface.ethos");
        let ordinary_source = fs::read_to_string(&ordinary_source_path)
            .expect("read the producer-owned ordinary Message Interface source");
        assert_eq!(
            ordinary_source,
            signal_message::MESSAGE_INTERFACE_SOURCE,
            "Cargo metadata must resolve the exact source compiled by signal-message",
        );

        let source_path = self.crate_root.join("ethos/interface.ethos");
        let rust_path = self.crate_root.join("src/schema/lib/generated.rs");
        let source = fs::read_to_string(&source_path).expect("read owner Message Interface source");
        ...
        .expect("assemble authority-approved owner Message Interface transaction");
        let rust = rust_logos();
        let type_paths = MessageRustTypePaths::new();

        BootstrapInterfaceGeneration::new(&assembly, &rust, &type_paths, &source_path, &rust_path)
            .generate()
            .expect("project owner Message Interface from the verified transaction")
            .write_or_check("META_SIGNAL_MESSAGE_UPDATE_INTERFACE_ARTIFACTS")
            .expect("checked-in owner Message Interface source and Rust projection are fresh");
        CargoEthosSourceMetadata::new("meta-signal-message")
...
fn imported_signal_seats() -> Vec<&'static signal_message::bootstrap_manifest::DeclarationSeat> {
    let wanted = IMPORTED_SIGNAL_TYPES
        .iter()
        .copied()
        .collect::<BTreeSet<_>>();
    let seats = signal_message::bootstrap_manifest::DECLARATION_SEATS
        .iter()
...
fn bootstrap_catalog() -> BootstrapCatalog {
    ...
    .expect("owner Message bootstrap catalog is complete")
}
...
struct MessageRustTypePaths(BTreeMap<VocabularyEncodedId, RustTypePath>);

impl MessageRustTypePaths {
    fn new() -> Self {
        use bootstrap_manifest as manifest;
...
impl RustTypePathResolver for MessageRustTypePaths {
