...
- Schema generation machinery lives in the TrueSchema `schema` and `schema-rust` producers.
...
- `build.rs` runs the TrueSchema `schema-rust` driver against the dependency
  schema and checks the checked-in artifacts for freshness.
